...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.technicalAssistanteSession.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.technical-assistante-sessions.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label for="client">{{ trans('cruds.technicalAssistanteSession.fields.client') }}</label>
                <input class="form-control {{ $errors->has('client') ? 'is-invalid' : '' }}" type="number" name="client" id="client" value="{{ old('client', '') }}" step="1">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('car') ? 'is-invalid' : '' }}" type="text" name="car" id="car" value="{{ old('car', '') }}">
                @if($errors->has('car'))
                    <div class="invalid-feedback">
                        {{ $errors->first('car') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('comercial') ? 'is-invalid' : '' }}" type="text" name="comercial" id="comercial" value="{{ old('comercial', '') }}">

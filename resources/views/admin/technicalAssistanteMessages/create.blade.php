...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.technicalAssistanteMessage.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.technical-assistante-messages.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('technical_assistante_session') ? 'is-invalid' : '' }}" name="technical_assistante_session_id" id="technical_assistante_session_id">
                    @foreach($technical_assistante_sessions as $id => $entry)
                        <option value="{{ $id }}" {{ old('technical_assistante_session_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\TechnicalAssistanteMessage::ROLE_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('role') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="role_{{ $key }}" name="role" value="{{ $key }}" {{ old('role', '') === (string) $key ? 'checked' : '' }}>

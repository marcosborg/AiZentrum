...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.aiAssistantIntruction.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.ai-assistant-intructions.update", [$aiAssistantIntruction->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('user') ? 'is-invalid' : '' }}" name="user_id" id="user_id" required>
                    @foreach($users as $id => $entry)
                        <option value="{{ $id }}" {{ (old('user_id') ? old('user_id') : $aiAssistantIntruction->user->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('user'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('ai_assistant_category') ? 'is-invalid' : '' }}" name="ai_assistant_category_id" id="ai_assistant_category_id" required>
                    @foreach($ai_assistant_categories as $id => $entry)
                        <option value="{{ $id }}" {{ (old('ai_assistant_category_id') ? old('ai_assistant_category_id') : $aiAssistantIntruction->ai_assistant_category->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $aiAssistantIntruction->name) }}" required>
                @if($errors->has('name'))
                    <div class="invalid-feedback">
                        {{ $errors->first('name') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('instructions') ? 'is-invalid' : '' }}" name="instructions" id="instructions">{!! old('instructions', $aiAssistantIntruction->instructions) !!}</textarea>

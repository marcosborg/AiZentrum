    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.ai-messages.update", [$aiMessage->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('client') ? 'is-invalid' : '' }}" type="number" name="client" id="client" value="{{ old('client', $aiMessage->client) }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('parent') ? 'is-invalid' : '' }}" name="parent_id" id="parent_id">
                    @foreach($parents as $id => $entry)
                        <option value="{{ $id }}" {{ (old('parent_id') ? old('parent_id') : $aiMessage->parent->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('email') ? 'is-invalid' : '' }}" type="text" name="email" id="email" value="{{ old('email', $aiMessage->email) }}">
                @if($errors->has('email'))
                    <div class="invalid-feedback">
                        {{ $errors->first('email') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('nif') ? 'is-invalid' : '' }}" type="text" name="nif" id="nif" value="{{ old('nif', $aiMessage->nif) }}">
                @if($errors->has('nif'))
                    <div class="invalid-feedback">
                        {{ $errors->first('nif') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('user') ? 'is-invalid' : '' }}" name="user_id" id="user_id" required>
                    @foreach($users as $id => $entry)
                        <option value="{{ $id }}" {{ (old('user_id') ? old('user_id') : $aiMessage->user->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('user'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('context') ? 'is-invalid' : '' }}" name="context" id="context">{{ old('context', $aiMessage->context) }}</textarea>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('ai_response') ? 'is-invalid' : '' }}" name="ai_response" id="ai_response">{{ old('ai_response', $aiMessage->ai_response) }}</textarea>
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\AiMessage::CONFLICT_TYPE_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('conflict_type') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="conflict_type_{{ $key }}" name="conflict_type" value="{{ $key }}" {{ old('conflict_type', $aiMessage->conflict_type) === (string) $key ? 'checked' : '' }} required>
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\AiMessage::URGENCY_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('urgency') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="urgency_{{ $key }}" name="urgency" value="{{ $key }}" {{ old('urgency', $aiMessage->urgency) === (string) $key ? 'checked' : '' }} required>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('resolved') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="resolved" id="resolved" value="1" {{ $aiMessage->resolved || old('resolved', 0) === 1 ? 'checked' : '' }}>
